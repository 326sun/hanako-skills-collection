...
--- # Apple Minimal UI Convey maximum hierarchy with minimum visual elements. Every pixel must justify its existence. ## Rules ### Color
...
- Accent pool: `#007AFF`, `#34C759`, `#FF6B35`, `#8B5CF6` (50% sat), or pure B&W. ### Typography
...
- Headings: `font-semibold/bold`, `tracking-tighter`. Body: `leading-relaxed` 1.6–1.75, `max-w-[65ch]`. ### Spacing
...
- Container: `max-w-7xl`. Hero: `min-h-[85dvh]`. ### Motion
...
- Page entrance: `opacity 0→1` + `translateY(8px→0)`, stagger 50–80ms. ### Components
...
- Icons: Phosphor or Heroicons, stroke-width 1.5. Never Lucide. ### Borders/Shadows
- Radius: page `rounded-3xl` → card `rounded-2xl` → button `rounded-xl` → badge `rounded-lg`.
- Shadows: `shadow-sm` max. Never `shadow-lg`/`shadow-2xl`. Replace heavy shadows with borders. ## Constraints **Forbidden**: gradient glows, centered large titles on dark grids, three equal feature cards, glassmorphism, Inter default, infinite-loop animations, gradient text, >1 accent, serif defaults, Lucide icons, motion >500ms, `scale` >1.05. **Quality checklist**: 1 accent? No gradient glows? Not Inter? Light shadows? Motion ≤400ms? Card padding ≥24px? No infinite loops? Dark bg = zinc-950? Consistent radii? No Lucide?
